...
# ---------------------------------------------------------------------------
# Unicode sanitiser — keeps Latin-1 characters, replaces common Unicode
# punctuation with ASCII equivalents so fpdf Helvetica never breaks.
# ---------------------------------------------------------------------------
...
def _s(text: str) -> str:
    """Sanitise a string for fpdf Helvetica output."""
    ...
    while i < len(lines):
        ...
        # Table row (pipe-delimited)
            ...
            if is_header:
                ...
                fill = False
            for c in cells:
                text_clean = re.sub(r"\*\*(.+?)\*\*", r"\1", c)
                text_clean = re.sub(r"\*(.+?)\*", r"\1", text_clean)
                pdf.cell(col_w, 6, _s(text_clean[:50]), border=1, fill=fill)
            pdf.ln()
...
def save_section_pdf(
    ...
    """Save a single curriculum section as its own PDF file."""
...
def save_full_curriculum_pdf(
    ...
    """
    Combine all sections and syllabi into one PDF.
    sections: ordered dict {section_title: markdown_content}
    syllabi: {course_code: markdown_content}
    """
    out_dir = _output_dir(base_outputs, institution, program_name)
...
def save_syllabus_pdf(
    ...
    """Save a single course syllabus as its own PDF."""
